...
- [x] 境界値・ランダム生成テストの追加→Hypothesis などで "ランダム合法手を生成 → 差分整合性チェック" を入れると手動で書ききれないエッジケースも網羅できる
- [ ] ネガティブテストをもう一歩深く。test_make_invalid_move は「同じマスを２回置く」だけ。他にも「ターゲット外のサブボードへ置く」「ゲーム終了後に置く」など異常系を網羅すると安心。

## 完了した改善事項

### プロパティベーステスト (`test_board_property.py`)

Hypothesisライブラリを使用したプロパティベーステストを追加し、手動では網羅困難なエッジケースの検証を実現：

1. **ランダム合法手生成テスト**：
   - 任意の手順でのゲーム進行における一貫性テスト
   - 座標変換の往復整合性検証
   - 合法手の妥当性検証
...
これらのテストにより、手動テストでは発見困難な以下のような問題を自動検出可能：
- 特定の手順でのみ発生する座標変換エラー
- 稀な盤面状態での合法手生成の不具合
- メモリ参照の意図しない共有
- 境界条件での計算エラー
